...
const arrayDeValidacao = [
  check('email')
    ...
    .isLength({ max: 50 })
    .withMessage('Campo recebe no máximo 50 caracteres')
    .custom(async (field) => {
  ...
  check('senha')
    ...
    .withMessage('Sua senhas precisa ter no mínimo 8 caracteres, sendo 2 letras minúsculas, 2 maiúsculas, 2 números e 1 caractere especial'), // .custom(async (field, req) => {
  //   const usuario = await Usuario.findOne({
  //     where: {
